...
namespace Cheetah.Kafka.Testing
{
    ...
    public class KafkaTestWriter<TKey, T> : IKafkaTestWriter<TKey, T>
    {
        ...
        public Task<DeliveryResult<TKey, T>[]> WriteAsync(params T[] messages)
        {
            ...
            var kafkaMessages = messages.Select(
                message => new Message<TKey, T> { Key = KeyFunction(message), Value = message, }
            );

            var produceTasks = kafkaMessages.Select(
                kafkaMessage => Producer.ProduceAsync(Topic, kafkaMessage)
            );
