...
namespace KafkaCore
{
    ...
    {
        private static void TokenRefreshHandler(ILogger logger, TokenService tokenService, IClient client)
        {
        ...
        private static CheetahKafkaTokenService BuildTokenService(ILogger logger, IServiceProvider provider) // We are not using DI, as we do not know which settings to look at.
        {
            ...
            var tokenService = new CheetahKafkaTokenService(logger, httpClientFactory, cache, oauthConfig);
            return tokenService;
        }
        ...
        public static ConsumerBuilder<TKey, TValue> AddCheetahOAuthentication<TKey, TValue>(this ConsumerBuilder<TKey, TValue> builder, IServiceProvider provider)
        {
            ...
            builder.SetOAuthBearerTokenRefreshHandler((client, cfg) => TokenRefreshHandler(logger, tokenService, client));
            return builder;
        }
